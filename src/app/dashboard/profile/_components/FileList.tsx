...
import { Loader2, Trash2, ChevronsUpDown, Clock, Edit, Users } from "lucide-react";
...
export default function FileList({
  ...
  useEffect(() => {
    const lookup = teamList.reduce((acc, team) => {
      ...
      {!isSmallScreen ? (
        <table className="min-w-full divide-y-2 overflow-hidden text-sm">
          ...
          <tbody className="divide-y divide-stone-600">
            ...
            {(sortedFiles.length > 0 ? sortedFiles : safeFileList).map((file) => (
              <FileRow
                key={file._id}
                file={file}
